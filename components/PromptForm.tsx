...
const promptTemplates = [
  {
    ...
    prompt:
      'A breathtaking, cinematic, ultra-realistic 4K drone shot of a majestic mountain range at sunrise. The golden light hits the peaks, casting long shadows in the valleys. A gentle mist rises from the pine forests below.',
  },
  {
    title: 'Sci-Fi Action Sequence',
    prompt:
      "An intense, fast-paced action scene in a futuristic neon-lit city. A sleek flying car weaves through towering skyscrapers, chased by glowing drones. Rain slicks the surfaces, reflecting the city's vibrant lights.",
  },
  {
    title: 'Whimsical Animation',
    prompt:
      'A charming, whimsical 3D animation of a small fox sipping tea in a cozy, cluttered burrow. Sunlight streams through a small window, illuminating dust particles in the air. The style is soft and heartwarming.',
  },
  {
    title: 'Abstract Visuals',
    prompt:
      'An abstract, mesmerizing video of colorful ink swirling in water, forming intricate patterns and shapes. The movement is slow and hypnotic, set against a dark background. Macro shot, high detail.',
  },
  {
    title: 'Nature Documentary',
    prompt:
      "A close-up, high-definition shot of a hummingbird in slow motion, its iridescent feathers shimmering as it hovers next to a vibrant flower. The background is softly blurred, focusing on the bird's delicate movements.",
  },
];

const veoModelDisplayInfo: Record<
  VeoModel,
  {name: string; description: string; icon: React.ReactNode}
> = {
  [VeoModel.VEO_FAST]: {
    name: 'Veo Fast',
    description: 'Quick generation, ideal for previews.',
    icon: <SparklesIcon className="w-5 h-5 text-yellow-400" />,
  },
  [VeoModel.VEO]: {
    name: 'Veo HD',
    description: 'Highest quality, for final renders and references.',
...
const PromptForm: React.FC<PromptFormProps> = ({
    ...
    try {
      ...
      alert(
        error instanceof Error
          ? error.message
          : 'An unknown error occurred while enhancing the prompt.',
      );
    } finally {
  ...
  const handleSelectModel = (newModel: VeoModel) => {
    setModel(newModel);
    setIsModelSelectorOpen(false);
  };
        ...
        <div className="flex items-end gap-2 bg-[#1f1f1f] border border-gray-600 rounded-2xl p-2 shadow-lg focus-within:ring-2 focus-within:ring-indigo-500">
          ...
          <div className="relative" ref={modelSelectorRef}>
            <button
              ...
              {veoModelDisplayInfo[model].icon}
              <span className="font-medium text-sm whitespace-nowrap">
                {veoModelDisplayInfo[model].name}
              </span>
            </button>
            {isModelSelectorOpen && (
              <div className="absolute bottom-full mb-2 w-72 bg-[#2c2c2e] border border-gray-600 rounded-lg shadow-xl overflow-hidden z-10">
                ...
                {Object.values(VeoModel).map((modelValue) => (
                  <button
                    key={modelValue}
                    type="button"
                    onClick={() => handleSelectModel(modelValue)}
                    className={`w-full text-left flex items-start gap-3 p-3 hover:bg-indigo-600/50 disabled:bg-gray-800 disabled:text-gray-500 disabled:cursor-not-allowed ${model === modelValue ? 'bg-indigo-600/30 text-white' : 'text-gray-300'}`}
                    disabled={isRefMode && modelValue !== VeoModel.VEO}>
                    {veoModelDisplayInfo[modelValue].icon}
                    <div className="flex-1">
                      <p className="font-semibold text-sm">
                        {veoModelDisplayInfo[modelValue].name}
                      </p>
                      <p className="text-xs text-gray-400">
                        {veoModelDisplayInfo[modelValue].description}
                      </p>
                    </div>
                  </button>
                ))}
          ...
          <div className="relative" ref={formatMenuRef}>
            <button
              ...
              className="flex shrink-0 items-center gap-2 px-3 py-2.5 rounded-lg hover:bg-gray-700 text-gray-300 hover:text-white transition-colors disabled:text-gray-500 disabled:hover:bg-transparent disabled:cursor-not-allowed"
            ...
            {isFormatMenuOpen && (
              <div className="absolute bottom-full mb-2 w-72 bg-[#2c2c2e] border border-gray-600 rounded-lg shadow-xl overflow-hidden z-10 p-4">
          ...
          <div className="relative group">
            <button
              ...
              className="p-2.5 rounded-full hover:bg-gray-700 text-gray-300 disabled:text-gray-600 disabled:cursor-not-allowed disabled:hover:bg-transparent"
            ...
            {!isEnhancing && (
              <div
                role="tooltip"
                className="absolute bottom-full right-1/2 translate-x-1/2 mb-2 w-max max-w-xs px-3 py-1.5 bg-gray-900 border border-gray-700 text-white text-sm rounded-lg shadow-lg opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none z-10">
          ...
          <div className="relative" ref={templatesRef}>
            <button
              ...
              className={`p-2.5 rounded-full hover:bg-gray-700 ${isTemplatesOpen ? 'bg-gray-700 text-white' : 'text-gray-300'}`}
              aria-label="Show prompt templates">
              <LightbulbIcon className="w-5 h-5" />
            </button>
            {isTemplatesOpen && (
              <div className="absolute bottom-full right-0 mb-2 w-[520px] max-w-[90vw] bg-[#2c2c2e] border border-gray-600 rounded-lg shadow-xl z-20">
                <div className="p-3 border-b border-gray-700">
                  <h3 className="text-sm font-semibold text-gray-200">
                    Prompt Templates
                  </h3>
                ...
                <div className="flex space-x-3 p-3 overflow-x-auto scrollbar-hide">
                  {promptTemplates.map((template, index) => (
                    <button
                      ...
                      className="flex-shrink-0 w-56 h-40 bg-gray-800/50 hover:bg-indigo-600/50 rounded-lg p-3 text-left transition-colors flex flex-col">
                      <p className="font-medium text-white text-sm flex-shrink-0">
                        {template.title}
                      </p>
                      <p className="text-xs text-gray-300 mt-1 font-normal whitespace-normal overflow-hidden flex-grow">
                        {template.prompt}
                      </p>
                    </button>
                  ))}
